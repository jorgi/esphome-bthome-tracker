...
#include "esphome/components/esp32_ble_tracker/esp32_ble_tracker.h" // Ensure this is included for ESPBTUUID and ESPBTDevice
...
void CustomBLEScanner::send_bthome_discovery_messages_for_device(BTHomeDevice *device_obj) {
    ...
    JsonDocument device_doc; // Use modern JsonDocument
    ...
    auto send_sensor_discovery = [&](const std::string& sensor_type, const std::string& unit, const std::string& dev_class, const std::string& state_class) {
        JsonDocument sensor_config_doc; // Use modern JsonDocument
...
bool CustomBLEScanner::parse_bthome_v2_device(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  if (it == this->bthome_devices_.end()) {
    ...
    // Pass the raw address (uint8_t*) to the BTHomeDevice constructor
  ...
  JsonDocument state_doc; // Use modern JsonDocument
  size_t offset = 1;
